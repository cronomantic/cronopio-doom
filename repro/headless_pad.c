...
int main(int argc, char** argv) {
    ...
    { FILE* sf=fopen(savepath,"rb"); if(sf){ fseek(sf,0,SEEK_END); long sz=ftell(sf); fseek(sf,0,SEEK_SET);
        if(sz>0){ cronopio_save_reserve(&console,(uint32_t)sz); console.save_len=(uint32_t)fread(console.save,1,console.save_cap,sf);} fclose(sf);} }
